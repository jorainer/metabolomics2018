...
graphics: yes
date: "24 June 2018; updated July 18 2019"
output:
...
of the `MSnbase` and `xcms` (version >= 3.3.1 is needed) packages available
through Bioconductor version 3.9. The packages can be installed with:
...
BiocManager::install(c("xcms", "MSnbase", "msdata", "magrittr"))
```
...
register(bpstart(MulticoreParam()), default = TRUE) 
```

```{r disable-parallel, echo = FALSE}
## Disable parallel processing for rendering - there seems to be an
## issue with macOS beta and parallel processing.
...
detection on the `Chromatograms` object using the default parameters for
centWave.
...
Apparently we did not find any chromatographic peaks in the submitted data
(shown by `"peaks: 0"` for all chromatograms). Looking at the default values for
the centWave parameters helps understanding why the peak detection failed:
...
Note that the `srn_chr` object contains, after peak detection, also all
identified chromatographic peaks. These can be accessed with the `chromPeaks`
function:
...
Columns `"column"` indicate in which sample the peak was identified,
`"row"` would define in which EIC (i.e. m/z - retention time slice) the peak was
detected. All the remaining columns characterize the identified peak.
...
As expected, higher intensity signals tend to scatter less in m/z dimension.
...
```{r plotChromPeaks, warning = FALSE, message = FALSE, fig.cap = "Location of the identified chromatographic peaks in the m/z - rt space." }
par(mfrow = c(1, 2))
...
files of our toy data set to visualize these differences. Remember that calling
`chromatogram` on an object **after** peak detection will always return also
identified chromatographic peaks. Thus we use `peakType = "none"` in the
plotting call below to skip highlighting of identified chromatographic peaks
(which would otherwise create a rather busy plot).
...
```{r  correspondence-bw, warning = FALSE, message = FALSE, fig.cap = "Define correspondence settings to separate Betaine and Valine peaks.", fig.width = 10, fig.height = 10 }
